...
public class FilletRecyclingImageView extends ImageView {
    ...
    private float defaultRatio = 0.357f;

    public FilletRecyclingImageView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        imageWidth = ScreenUtil.getScreenWidth(context) - ScreenUtil.dp2px(context,15) * 2;
...
//        typedArray.recycle();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mRoundRect.set(0.0F, 0.0F, imageWidth, imageHeight);
        if (changed && radius > 0.0F && imageHeight > 0) {
            getRoundBitmap(imageWidth, imageHeight);
        }
    }
}
